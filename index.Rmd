...
- Start each project with a preprocessing action that formats `.feather` files and outputs `.rds` files which can be saved with `saveRDS()`
...
#### Tables
  ...
  - Use `write.csv()` to save `.csv` files that will be sent to output checkers (this is in the **utils** package which is loaded by default in every R session)
